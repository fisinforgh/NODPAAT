...
for ((c = 1979; c <= 1993; c++)); do
  mkdir nimbus_$c
  cd nimbus_$c
  wget -q -O directory_listing.html https://acdisc.gsfc.nasa.gov/data/s4pa/Nimbus7_TOMS_Level3/TOMSN7L3dtoz.008/$c/
  grep -o 'href="[^"]*\.txt"' directory_listing.html |
    grep -v '\.1\.txt' |
    cut -d'"' -f2 |
    while read file; do
      echo "https://acdisc.gsfc.nasa.gov/data/s4pa/Nimbus7_TOMS_Level3/TOMSN7L3dtoz.008/$c/$file"
