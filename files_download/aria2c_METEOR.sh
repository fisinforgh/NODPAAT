...
for ((c = 1991; c <= 1994; c++)); do
  mkdir meteor_$c
  cd meteor_$c
  wget -q -O directory_listing.html https://acdisc.gsfc.nasa.gov/data/s4pa/Meteor3_TOMS_Level3/TOMSM3L3dtoz.008/$c/

  grep -o 'href="[^"]*\.txt"' directory_listing.html |
    grep -v '\.1\.txt' |
    cut -d'"' -f2 |
    while read file; do
      echo "https://acdisc.gsfc.nasa.gov/data/s4pa/Meteor3_TOMS_Level3/TOMSM3L3dtoz.008/$c/$file"
