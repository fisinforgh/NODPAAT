...
void usage(){
  cout << "-A<latitude> -B<longitude> -P<prefix i.e BOG> -D</path/to/data> -S<opt>" << endl;
...
int main(int argc, char *argv[])
{
  ...
  strcat(cmdChkDir," ; echo $? > logChkDir.txt");
  
  system(cmdChkDir);

  inLogChkDir.open("logChkDir.txt");
  inLogChkDir >> chkDir;
  inLogChkDir.close();
  if(chkDir != 0){
    cerr << "path to data does not exist. Please check.." << endl;
    system("rm logChkDir.txt");
    exit(8);
  }
  else{
  ...
  char cnLine[100];
  rnLine = rLonBin/bpLine + 1; // find the line number from 1 to 12 (25 bins times 12 = 300)
  ...
  for(int i=YMIN; i<=YMAX; i++){
    ...
    strcat(cmdName, "/L3*.txt > list.txt"); 
	...
	strcat(cmdSed,cLatHalf);
      ...
      strcat(cmdSed," > logLatLine.txt");
      system(cmdSed);

      //latLine = 0;
      inLogLatLine.open("logLatLine.txt");
      inLogLatLine >> latLine;
      inLogLatLine.close();
      system("rm logLatLine.txt");
	...
	nLine = nLine -1;
      ...
      inLogLonLine.open("logLonLine.txt");
      getline(inLogLonLine,sLine);
      inLogLonLine.close();
      system("rm logLonLine.txt");
	...
	strBin = sLine.substr(73,3);
      ...
      strBin = sLine.substr((rLonBin - (rLonBin/bpLine)*25)*3 - 3 + 1,3);
      }
      cout << "ud: " << strBin << endl;
