...
const Int_t YMax = 2024; // change this year according to your study
...
int main(int argc, char *argv[]) {
  ...
  //===== READING FLAGS arguments (c++ executable command line options ===//

  if (argc != 4) {
    cerr << "Usage: ./chi2LRSO3vsSnRunApp -E<nEveOffSet> -N<preLoc> -I<conf. "
            "interval>"
         << endl;
    return 1;
  }
  ...
  cout << "Number of events to cutoff: " << nEvOffSet << endl;
  cout << "location LAT<>LON<>: " << preLoc << endl;
  ...
  gStyle->SetOptFit(
      1111); // https://root.cern.ch/doc/master/classTPaveStats.html
  gStyle->SetOptStat("e");
  ...
  // --- Create ROOT file in skim_preloc folder ---
  std::string rootFileName = std::string(dirName) + "asvssn.root";
  ...
  // --- Histograms ---
  TH1F *sn_vs_dd =
      new TH1F("sn_vs_dd", "Sunspot Number", binSnMax, 0, binSnMax);
  TH1F *snskim_vs_dd =
      new TH1F("snskim_vs_dd", "Sunspot Number", arrMax, 0, arrMax);
  ...
  while (!inSnFile.eof()) {
    ...
    if (snYY % 10 ==
        0) { // this is for setting labels everi 10 years, for 01JAN
      sprintf(lYY, "%d", snYY);
      if ((snMM == 1) && (snDD == 1)) {
  ...
  while (!inSnFileSkim.eof()) {
    ...
    if (snSkim[nSnSkim] == 270) {
      // cout << snYY << snMM << snDD << " ------------>SD 270: " <<
      // snSkimSD[nSnSkim] << endl;
    }
  ...
  // --- Build sort and log paths inside dirName ---
  char sortFile[500];
  strcpy(sortFile, dirName);
  strcat(sortFile, "sortfile.dat");

  char logSortFile[500];
  strcpy(logSortFile, dirName);
  strcat(logSortFile, "logSort.txt");

  // --- Build system command for sorting ---
  char cmdSort[1000];
  strcpy(cmdSort, "sort -n ");
  strcat(cmdSort, outFileName);
  strcat(cmdSort, " > ");
  strcat(cmdSort, sortFile);

  system(cmdSort);

  // Instead of using fixed names, prepend dirName
  char cmdHead[1000];
  sprintf(cmdHead, "head -1 %s > %s", sortFile, logSortFile);
  system(cmdHead);

  char cmdTail[1000];
  sprintf(cmdTail, "tail -1 %s >> %s", sortFile, logSortFile);
  system(cmdTail);

  char cmdWc[1000];
  sprintf(cmdWc, "wc -l %s >> %s", sortFile, logSortFile);
  system(cmdWc);
  ...
  inSortLines.open(logSortFile);
  inSortLines >> initSn;
  inSortLines >> auxDat;
  inSortLines >> auxDat; // from third column on SD of sortfile.dat
  inSortLines >> clastSn;
  inSortLines >> auxDat;
  inSortLines >> auxDat; // from third column on SD of sortfile.dat
  ...
  // --- Clean up log file ---
  char cmdRm[1000];
  sprintf(cmdRm, "rm %s", logSortFile);
  system(cmdRm);
  ...
  Double_t udMax[lastSn + 1], udMin[lastSn + 1], absDS[lastSn + 1],
      ev[lastSn + 1];
  Int_t contEr = -1;
  ...
  Double_t erSnSkim[lastSn + 1], erUdSkim[lastSn + 1], erXSkim[lastSn + 1],
      erYSkim[lastSn + 1];
  Double_t udMaxSkim[lastSn + 1], udMinSkim[lastSn + 1], absDSSkim[lastSn + 1],
      evSkim[lastSn + 1];
  Int_t contErSkim = -1;

  // --- Open sorted file inside dirName ---
  inSortFile.open(sortFile);
  ...
  while (!inSortFile.eof()) {
    ...
    if (sortSn == initSn) {
      ...
      if (nEv > 1) {
        ...
        // SSE: sum of squared of errors Variance(Sn)= SSE/(n-1) SD=
        // sqrt(Variance)
        ...
        erY[contEr] = alpha * 1.0 / (TMath::Sqrt((Double_t)nEv)) *
                      TMath::Sqrt(1.0 / ((Double_t)nEv - 1.0) * var_s2);
        // cout << "S2. " << var_s2 << nEv << " ey: " << erY[contEr] << endl;
        // cout << 1.0/(TMath::Sqrt((Double_t)nEv)) << " " << 1.0/(nEv-1.0) <<
        // endl;
      ...
      outFileErr << erSn[contEr] << "\t" << erUd[contEr] << "\t" << erX[contEr]
                 << "\t" << erY[contEr] << "\t\t" << udMax[contEr] << "\t"
                 << udMin[contEr] << "\t" << ev[contEr] << "\t" << absDS[contEr]
                 << endl;
      ...
      if (nEv > nEvOffSet) {
        if (erY[contEr] > 0) {
          ...
          outFileErrSkim << erSnSkim[contErSkim] << "\t" << erUdSkim[contErSkim]
                         << "\t" << erXSkim[contErSkim] << "\t"
                         << erYSkim[contErSkim] << "\t" << udMaxSkim[contErSkim]
                         << "\t" << udMinSkim[contErSkim] << "\t"
                         << evSkim[contErSkim] << "\t" << absDSSkim[contErSkim]
                         << endl;
        }
      }
  ...
  char cmdMv[1000];
  sprintf(cmdMv, "mv %s %s%s_sortSno3.dat", sortFile, dirName, preLoc);
  system(cmdMv);
  ...
  TGraphErrors *grSnUdAvErSkim =
      new TGraphErrors(contErSkim, erSnSkim, erUdSkim, erXSkim, erYSkim);
  ...
  cout << "All events: chi2/ndf " << fitFcn->GetChisquare() / fitFcn->GetNDF()
       << endl;
  ...
  cout << "Events cut off  " << nEvOffSet
       << " chi2/ndf: " << fitFcn->GetChisquare() / fitFcn->GetNDF() << endl;
  ...
  // Avoiding malloc problems since this program will be standalone version cxx
  file->Close("R"); // It is very important to close files!!!! otherwise
                    // malloc() problems.

  sprintf(cmdMv, "mv %s %s%s_sno3av.root", rootFileName.c_str(), dirName,
          preLoc);
  system(cmdMv);

  cout << "lastSn: " << lastSn << endl;

  // because int main funtion
  return 0;
