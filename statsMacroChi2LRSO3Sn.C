...
void statsMacroChi2LRSO3Sn(int h, int pal, int eve) {
  ...
  cout << "Starting file processing loop..." << endl;
  cout << "Grid size: " << h << "x" << h << ", Event cutoff: " << eve << endl;
  cout << "Expected files to process: "
       << ((lonMax - lonMin) / h) * ((latMax - latMin) / h + 1) << endl;
  ...
  for (int i = lonMin; i < lonMax; i = i + h) {
    for (int j = latMin; j <= latMax; j = j + h) {
      ...
      sprintf(dirPathName, "skim_LAT%dLON%d/LAT%dLON%d_fitlinear.dat", j, i, j,
              i);

      // DEBUG OUTPUT - Progress tracking
      if (nCont % 100 == 0) {
        cout << "Processing file #" << nCont << ": " << dirPathName << endl;
      }

      inChi2File.open(dirPathName);

      // ERROR CHECKING - File opening
      if (!inChi2File.is_open()) {
        cout << "ERROR: Could not open file #" << nCont << ": " << dirPathName
             << endl;
        cout << "Current coordinates: LAT=" << j << " LON=" << i << endl;
        continue; // Skip this file and continue with next one
      }

      // SUCCESSFUL OPEN DEBUG (less frequent)
      if (nCont % 500 == 0) {
        cout << "Successfully opened file #" << nCont << " at LAT=" << j
             << " LON=" << i << endl;
      }
      ...
      for (int rows = 0; rows < 1; rows++) {
        ...
        if (inChi2File.fail()) {
          cout << "WARNING: Failed to read data from file #" << nCont << ": "
               << dirPathName << endl;
          break;
        }
  ...
  cout << "COMPLETED FILE LOOP: Processed " << nCont << " files total" << endl;

  Int_t x, y;
  int z;

  cout << "Opening earth.dat file..." << endl;
  inEarthData.open("earth.dat");

  if (!inEarthData.is_open()) {
    cout << "ERROR: Could not open earth.dat file" << endl;
    return;
  }
  ...
  while (!inEarthData.eof()) {
    ...
    if (nn % 10000 == 0) {
      cout << "Read " << nn << " lines from earth.dat" << endl;
    }
  }
  inEarthData.close();
  cout << "Finished reading earth.dat: " << nn << " lines processed" << endl;

  cout << "Writing statistics to output file..." << endl;
  ...
  outFile << endl;
  outFile << "suma: "
          << chi2Count0 + chi2Count0_5 + chi2Count0_75 + chi2Count1 +
                 chi2Count1_25 + chi2Count1_5 + chi2Count1_75 + chi2Count2
          << endl;
  outFile << "nCont: " << nCont << endl;
  int press = 4;
  outFile << endl;
  outFile << "chi entre 0 y 0.5: " << setprecision(press)
          << (float)chi2Count0 / (float)nCont * 100 << "%" << endl;
  outFile << "chi entre 0.5 y 0.75: " << setprecision(press)
          << (float)chi2Count0_5 / (float)nCont * 100 << "%" << endl;
  outFile << "chi entre 0.75 y 1.0: " << setprecision(press)
          << (float)chi2Count0_75 / (float)nCont * 100 << "%" << endl;
  outFile << "chi entre 1.0 y 1.25: " << setprecision(press)
          << (float)chi2Count1 / (float)nCont * 100 << "%" << endl;
  outFile << "chi entre 1.25 y 1.5: " << setprecision(press)
          << (float)chi2Count1_25 / (float)nCont * 100 << "%" << endl;
  outFile << "chi entre 1.5 y 1.75: " << setprecision(press)
          << (float)chi2Count1_5 / (float)nCont * 100 << "%" << endl;
  outFile << "chi entre 1.75 y 2 " << setprecision(press)
          << (float)chi2Count1_75 / (float)nCont * 100 << "%" << endl;
  outFile << "chi entre 2 y mas " << setprecision(press)
          << (float)chi2Count2 / (float)nCont * 100 << "%" << endl;
  outFile << endl;
  ...
  outFile << "ProbCut entre 0 y 0.025: " << setprecision(press)
          << (float)probCont0_2p5 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.025 y 0.05: " << setprecision(press)
          << (float)probCont2p5_5 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.025 y 0.975: " << setprecision(press)
          << (float)probCont2p5_97p5 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.05 y 0.95: " << setprecision(press)
          << (float)probCont5_95 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.95 y 1: " << setprecision(press)
          << (float)probCont95_1 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.975 y 1: " << setprecision(press)
          << (float)probCont97p5_1 / (float)nCont * 100 << "%" << endl;
  outFile << endl;
  ...
  outFile << endl;
  outFile << "ProbCut entre 0 y 0.1:   " << probCont1 << " "
          << (float)probCont1 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.1 y 0.2: " << probCont2 << " "
          << (float)probCont2 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.2 y 0.3: " << probCont3 << " "
          << (float)probCont3 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.3 y 0.4: " << probCont4 << " "
          << (float)probCont4 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.4 y 0.5: " << probCont5 << " "
          << (float)probCont5 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.5 y 0.6: " << probCont6 << " "
          << (float)probCont6 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.6 y 0.7: " << probCont7 << " "
          << (float)probCont7 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.7 y 0.8: " << probCont8 << " "
          << (float)probCont8 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.8 y 0.9: " << probCont9 << " "
          << (float)probCont9 / (float)nCont * 100 << "%" << endl;
  outFile << "ProbCut entre 0.9 y 1.0: " << probCont10 << " "
          << (float)probCont10 / (float)nCont * 100 << "%" << endl;
  ...
  outFile << "Total probCur: "
          << probCont1 + probCont2 + probCont3 + probCont4 + probCont5 +
                 probCont6 + probCont7 + probCont8 + probCont9 + probCont10
          << endl;
  outFile.close();

  cout << "Dat file ready: " << fileStatsName << endl;

  cout << "Creating graphs and canvases..." << endl;
  ...
  cout << "All canvases and graphs created successfully!" << endl;
  cout << "ROOT file saved as: " << fileROOTName << endl;
}
